...
func (p *GroqProvider) Generate(ctx context.Context, opts *GenerateOptions) (*GenerateResult, error) {
	model := opts.Model
	if model == "" {
...
func (p *GroqProvider) GenerateStream(ctx context.Context, opts *GenerateOptions) (<-chan string, error) {
	model := opts.Model
	if model == "" {
		model = p.defaultModel
	}
